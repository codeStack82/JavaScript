...
function verifyMonths() {
    ...
    if(monthsBox.value > 0 ){
        monthsComplete = true;
        testFormCompleteness();
        messageElement.innerHTML = "3 of 4 fields have been filled out. Only 1 more to Go!";
    }
}

/* verify that a fuel option button is selected */
function verifyFuel() {
    //alert("Fuel input changed")
...
function testFormCompleteness() {
   if (acresComplete && cropsComplete && monthsComplete && fuelComplete) {
      //alert("test completness true!")
      createRecommendation();
  }//else{   alert("test completness false!")}
...
function createEventListeners() {
   ...
   for (var i = 0; i < 7; i++) {
      ...
      if (cropsBox.addEventListener) {
        ...
        //alert("event calling verify crops func" + " " +i)
...
if (window.addEventListener) {
   ...
   window.attachEvent("onload", testAll);
}
